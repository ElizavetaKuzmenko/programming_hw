...
fpath = '/home/liza/PycharmProjects/programming_hw/hw_tf_itf/09'
...
def read_corpus():
    ...
    for t in files:
        with open(fpath + os.sep + t) as f:
            ...
            lemmas = [lemma for lemma in lemmas if lemma not in punct]
            texts.append(lemmas)
    return texts
...
def tf_idf(texts):
    ...
    for textnum in range(len(texts)):
        tf_dict = {}
        for lemma in texts[textnum]:
            try:
                tf_dict[lemma] += 1
            except KeyError:
                tf_dict[lemma] = 1
        for lemma in tf_dict:
            tf_idf_dict[(textnum, lemma)] = 1 + log10(tf_dict[lemma])*idf_dict[lemma]
...
if __name__ == '__main__':
    ...
    for lemma_doc in sorted(tf_idf_dict, key=lambda lemma_doc: tf_idf_dict[lemma_doc], reverse=True)[:10]:
        print(lemma_doc[0], lemma_doc[1], tf_idf_dict[lemma_doc], sep=' - ')